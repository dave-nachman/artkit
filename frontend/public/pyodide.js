...
var languagePluginLoader = new Promise((resolve, reject) => {
  ...
  // } else {
  //   console.log("a");
  //   wasm_promise = new Promise(resolve => {
  //     localforage.getItem(key, (e, v) => {
  //       if (v) {
  //         console.log("got precompiled");
  //         resolve(v);
  //       } else {
  //         WebAssembly.compileStreaming(fetch(wasmURL)).then(x => {
  //           localforage.setItem(key, x);
  //           console.log("set compilation");
  //           resolve(x);
  //         });
  //       }
  //     });
  ...
  Module.instantiateWasm = (info, receiveInstance) => {
    WebAssembly.instantiateStreaming(fetch(wasmURL), info).then(response => {
      console.log(Date.now() - start);
      return receiveInstance(response.instance);
    });
